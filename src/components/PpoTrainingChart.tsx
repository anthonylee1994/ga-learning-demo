...
import type {PpoUpdateStats} from "../domains/breaker/ppo";

interface Props {
    history: PpoUpdateStats[];
}

export const PpoTrainingChart = React.memo<Props>(({history}) => {
    return (
        <div className="chart-panel">
            <div className="panel-heading">
                ...
                <div className="legend-row">
                    ...
                    固定評估
                    <span className="legend-dot ppo-loss" />
                    策略 loss
                </div>
            </div>
                ...
                {history.length > 1 ? (
                    <ResponsiveContainer height="100%" width="100%">
                        <LineChart data={history} margin={{left: -18, right: 4, top: 8, bottom: 0}}>
                            ...
                            <Line dataKey="policyLoss" dot={false} isAnimationActive={false} name="策略 loss" stroke="#7aa2c9" strokeDasharray="4 3" strokeWidth={1.4} type="monotone" yAxisId="loss" />
                        </LineChart>
                    </ResponsiveContainer>
                ) : (
                    <div className="empty-chart">開始訓練後，呢度會顯示每輪 PPO 更新嘅回報同 loss。</div>
                )}
            </div>
        </div>
    );
