...
describe("Futu Python export", () => {
    it("embeds optimized parameters, network weights, and state-machine entries", () => {
        ...
        expect(script).toContain("buy_signal = out_buy >= stay_buy + ACTION_MARGIN");
        expect(script).toContain("sell_signal = False");
        expect(script).not.toContain("MIN_BARS_LONG");
        expect(script).not.toContain("bars_in_state");
        expect(script).toContain("if buy_signal and position < 1:");
        expect(script).toContain("elif sell_signal and position > 0:");
        expect(script).toContain("position = 1");
        expect(script).toContain("position = 0");
        expect(script).not.toContain("position = -1");
        ...
        expect(script).toContain("Shape.arrowup");
        expect(script).toContain("Shape.arrowdown");
