...
describe("Pine Script export", () => {
    it("embeds optimized parameters, signal weights, and long-only orders", () => {
        ...
        expect(script).toContain("f12 = clamp(volumeZScore / 3.0)");
        expect(script).toContain("tanh(value) =>");
        expect(script).toContain("exponent = math.exp(2.0 * limited)");
        expect(script).toContain("score = tanh(");
        expect(script).toContain("buySignal = ready and score > enterThreshold");
        expect(script).toContain("sellSignal = ready and score < exitThreshold");
        expect(script).not.toContain("math.tanh");
        expect(script).not.toContain("h1_0");
